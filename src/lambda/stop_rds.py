...
def shut_rds_all():
    ...
    for i in response['DBInstances']:
        # Filters aurora clusters from single instance databases as boto3 commands defer to stop the aurora clusters.
        if i['Engine'] not in ['aurora-mysql','aurora-postgresql']:
            # Filters Read replicas.
            if i['DBInstanceIdentifier'] not in v_readReplica and len(i['ReadReplicaDBInstanceIdentifiers']) == 0:
                ...
                # RDS instance is part of the Auto-Shutdown group.
                if 0==len(resp2['TagList']):
                    ...
                    for tag in resp2['TagList']:
                        # The tags match, then stop the instances by validating the current status.
